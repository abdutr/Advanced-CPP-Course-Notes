...
// auto main() -> int {
//     "123,456/2"_dt; // This works thanks to the second size_t
//     // size_t parameter version is required to distinguish between string literals and uncooked version\

//     // Generally the cooked version is preferred, but sometimes it does not satisfy the use case
//     // Let's say we want very high-precision data and don't want precision loss due to conversion to long
...
//     int x  = 3.56; // Valid, warning, not great
...
//     // String view is not null terminated, so we cannot use it in contexts that require null-terminated strings
    
//     // String view does not modify original string, it has some non const functions like
//     //  remove_prefix, remove_suffix, but they do not modify the original string; they just change the view
...
//     // Accessing an empty optional value using *ox or ox->length() is UB
//     // Accessing an empty optional value using ox.value() throws an exception if empty\
//     // Bad optional access
// }