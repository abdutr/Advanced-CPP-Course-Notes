...
auto main() -> int {
    ...
    *op ;// does not throw exception, UB
    ...
    std::optional<std::reference_wrapper<std::string>> op2{ref(name0)}; // Reference wrapper works
    op2->get() += " Uysal";
    std::cout << name0 << "\n";
    ...
    if(auto op = getPersonNick(5);op->length()> 10){
    ...
    cout << std::holds_alternative<int>(vx) << "\n"; // false
    cout << std::holds_alternative<float>(vx) << "\n"; // false
    ...
    cout << var10.index() << "\n"; // 0
    cout << std::holds_alternative<monostate>(var10)<< "\n"; // 0
