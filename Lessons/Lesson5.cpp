...
// details
template <typename T> void processContainer(T &&coll) {
  navigate(coll.begin(), coll.end());
}

template <typename T> void func(T &, T &) {};

template <typename T> void insert1(std::vector<T> &vec, T &&element) {
...
template <typename C, typename T> void Fill(C &con, const T &val) {
...
template <typename T>
decltype(auto)
perfectReturnFunc(T &&t) { // Allow us to return with exact return type of foo
...
auto main() -> int {
  ...
  foo(MyClass{});        // This is same with 2 line belove
  auto &&var1 = MyClass{};
